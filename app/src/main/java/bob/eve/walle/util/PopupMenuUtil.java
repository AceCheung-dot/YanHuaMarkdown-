...
import bob.eve.walle.ui.activity.InsertTypeActivity;
    ...
    private class MViewClick implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (index == 0) {
                ...
                if(index==2||index==3){
                    value=1;//垃圾桶
                    Intent i2=new Intent(context, insertActivity.class);
                    i2.putExtra("mstate", value);
                    i2.putExtra("mlatitude",MyUser.getMlatitude());
                    i2.putExtra("mlongtitude", MyUser.getMlatitude());
                    i2.putExtra("myplace", MyUser.getMyplace());
                    context.startActivity(i2);
                }

                else if(index==6){//其他标注类型
                   Toast.makeText(context,"其他标注类型",Toast.LENGTH_LONG).show();
                }
                else {//新增
                   Intent i=new Intent(context, InsertTypeActivity.class);
